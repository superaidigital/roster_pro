...
class SettingsController {
    ...
    // ========================================================
    // 👑 ส่วนจัดการเมนู (เฉพาะ SUPERADMIN)
    // ========================================================
    ...
    // ========================================================
    // 🌟 ระบบสำรองฐานข้อมูล (Database Backup)
    // ========================================================
    ...
    public function do_backup() {
        ...
        // ขยายลิมิตเซิร์ฟเวอร์สำหรับข้อมูลขนาดใหญ่
        set_time_limit(300); 
        ...
        try {
            ...
            // บันทึกประวัติ Logs เพื่อให้หน้า Backup โชว์ข้อมูล
    ...
    public function factory_reset() {
        ...
        // ตรวจสอบความถูกต้องของรหัสยืนยันอีกชั้นในฝั่ง Server
        ...
        try {
            // 🌟 ไม่อนุญาตให้ใช้ beginTransaction() กับคำสั่ง TRUNCATE TABLE เพราะ TRUNCATE เป็น DDL
            // ซึ่งจะทำให้ Transaction ปัจจุบันถูกยืนยัน (Commit) ไปอัตโนมัติ จึงนำ Transaction ออกเพื่อป้องกัน Error หน้าขาว
            
            // 1. ปิดการตรวจสอบ Foreign Key ชั่วคราวเพื่อให้ TRUNCATE ได้
            $db->exec("SET FOREIGN_KEY_CHECKS=0;");

            // 2. กำหนดรายชื่อตารางที่เป็น "ข้อมูลปฏิบัติการ" ที่ต้องล้างทิ้ง
            $tables_to_clear = [
                'shifts',             // ตารางเวร
                'rosters',            // ข้อมูลตารางเวรแต่ละเดือน
                'roster_details',     // รายละเอียดเวรแต่ละวัน
                'roster_status',      // สถานะและยอดเงินสรุป
                'leaves',             // การลางาน
                'leave_requests',     // คำขอการลา
                'shift_swaps',        // การแลกเวร
                'logs',               // ประวัติการใช้งาน
                'system_logs',        // ประวัติของแอดมิน
                'notifications'       // ล้างแจ้งเตือนทั้งหมด
            ];

            foreach ($tables_to_clear as $table) {
                // ตรวจสอบว่าตารางมีอยู่จริงในฐานข้อมูลก่อนสั่งลบ
            ...
            // รีเซ็ตยอดวันลาที่ใช้ไปให้เป็น 0 แต่ยังคงโควตาไว้
            $stmt = $db->query("SHOW TABLES LIKE 'leave_balances'");
            if ($stmt->rowCount() > 0) {
                $db->exec("UPDATE leave_balances SET used_days = 0, carried_over_days = 0");
            }

            // 3. เปิดการตรวจสอบ Foreign Key กลับเป็นปกติ
            $db->exec("SET FOREIGN_KEY_CHECKS=1;");

            // 4. บันทึกประวัติ (Logs จะถูกล้างไปแล้ว อันนี้จะเป็น Log แถวแรกของระบบใหม่)
            ...
            $db->exec("SET FOREIGN_KEY_CHECKS=1;"); // กันเหนียวกรณี Error
